...
contract AIJobManagerV2 is Ownable {
    ...
    struct Job { uint256 id; address user; uint256 agentId; uint256 computeNodeId; uint256 requestId; uint256 reward; uint256 createdAt; uint256 startedAt; uint256 finishedAt; JobStatus status; }
    ...
    function setGateway(address gatewayAddress) external onlyOwner { require(gatewayAddress != address(0), "Job: zero gateway"); gateway = gatewayAddress; emit GatewayUpdated(gatewayAddress); }
    ...
    function setContracts(address schedulerAddress, address runtimeAddress, address poolAddress, address oracleAddress) external onlyOwner {
        ...
        scheduler = AIScheduler(schedulerAddress); runtime = AIAgentRuntimeV2(runtimeAddress); computePool = AIComputePoolV2(poolAddress); reputationOracle = AIReputationOracleV2(oracleAddress);
        emit ContractsConnected(schedulerAddress, runtimeAddress, poolAddress, oracleAddress);
    }

    function createJob(uint256 agentId, uint256 reward) external returns (uint256) { return _createJob(msg.sender, agentId, 0, reward); }

    function createJobFromGateway(address user, uint256 agentId, uint256 requestId, uint256 reward) external returns (uint256) {
    ...
    function _createJob(address user, uint256 agentId, uint256 requestId, uint256 reward) internal returns (uint256 jobId) {
        ...
        if (address(scheduler) != address(0)) { scheduler.scheduleJob(jobId, agentId); jobs[jobId].status = JobStatus.Scheduled; }
    }

    function assignNode(uint256 jobId, uint256 nodeId) external onlyOwner {
        Job storage job = jobs[jobId]; require(job.createdAt != 0, "Job: not found"); require(job.status == JobStatus.Created || job.status == JobStatus.Scheduled, "Job: bad status");
        computePool.getNode(nodeId); job.computeNodeId = nodeId; job.status = JobStatus.Scheduled; emit JobNodeAssigned(jobId, nodeId);
    }

    function startJob(uint256 jobId) external onlyOwner {
        Job storage job = jobs[jobId]; require(job.createdAt != 0, "Job: not found"); require(job.status == JobStatus.Scheduled, "Job: not scheduled"); require(job.computeNodeId != type(uint256).max, "Job: node missing");
        job.startedAt = block.timestamp; job.status = JobStatus.Running; runtime.startAgent(job.agentId); computePool.startJob(jobId, job.computeNodeId); emit JobStarted(jobId);
    }

    function finishJob(uint256 jobId) external onlyOwner {
        Job storage job = jobs[jobId]; require(job.createdAt != 0, "Job: not found"); require(job.status == JobStatus.Running, "Job: not running");
        uint256 reward = job.reward; job.finishedAt = block.timestamp; job.status = JobStatus.Finished; job.reward = 0;
        rewardToken.safeTransfer(address(computePool), reward);
    ...
    function failJob(uint256 jobId) external onlyOwner {
        Job storage job = jobs[jobId]; require(job.createdAt != 0, "Job: not found"); require(job.status == JobStatus.Running || job.status == JobStatus.Scheduled, "Job: bad status");
        bool wasRunning = job.status == JobStatus.Running; uint256 refund = job.reward; job.status = JobStatus.Failed; job.reward = 0;
        if (wasRunning) computePool.failJob(jobId, job.computeNodeId);
        if (refund > 0) rewardToken.safeTransfer(job.user, refund);
        reputationOracle.processFailedJob(job.agentId); emit JobFailed(jobId, job.computeNodeId);
    }
}
