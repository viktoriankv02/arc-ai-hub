...
def prepare(opportunity_id: str, task_id: str):
    try:
        return prepare_task(opportunity_id, task_id)
    except KeyError as exc:
        raise HTTPException(404, str(exc))
...
def approve(opportunity_id: str, task_id: str, req: ApprovalRequest):
    try:
        return approve_task(opportunity_id, task_id, req.approved)
    except KeyError as exc:
        raise HTTPException(404, str(exc))
...
def feedback(req: FeedbackRequest):
    return {"ok": True, "record": record_feedback(req.opportunity_id, req.outcome, req.note)}
